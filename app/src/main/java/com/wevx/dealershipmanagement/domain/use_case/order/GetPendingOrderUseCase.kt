...
class GetPendingOrderUseCase @Inject constructor(
    ...
    operator fun invoke(customerId: String): Flow<Resource<List<PendingOrderModel>>> = flow {
        try {
            emit(Resource.Loading())

            val response = orderRepository.getPendingOrdersByCustomer(customerId)

            if (response.isSuccessful) {
                val body = response.body()
                val responseList = body?.data ?: emptyList()
                val data = responseList.toPendingOrderModelList()

                emit(Resource.Success(data = data))
            } else {
                emit(Resource.Error("Pending order fetch failed: ${response.message()}"))
            }
