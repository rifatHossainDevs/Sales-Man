...
import com.wevx.dealershipmanagement.data.dto.homeDto.getStoreOwnerByAreaDTO.toStoreOwnerModelList
...
class GetStoreOwnerByAreaUseCase @Inject constructor(
    private val storeOwnerRepository: StoreOwnerRepository
) {
    operator fun invoke(areaId: Int): Flow<Resource<List<StoreOwnerModel>>> = flow {
        try {
            emit(Resource.Loading())

            val response = storeOwnerRepository.getStoreOwnerByArea(areaId)
